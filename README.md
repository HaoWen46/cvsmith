...
**v0.1.2** — ten external review rounds folded in; 311 planted-fixture tests:
...
- [x] **Review hardening (v0.1.2)** — Six further review rounds, every finding reproduced before fixing (round 6: excluded lines within Basics no longer count as identity evidence; auxiliary negations governing an achievement verb — "did not reduce" — now caught; a correctly-supported claim is no longer rejected by an unrelated negated line elsewhere; a wrong candidate surname FAILs instead of warning; `meta.target_level` fully operationalized — schema, type/enum validation, seniority-bar rubric, level-read in the report; the flagship fixture persists `meta.template`/`meta.bullet_lines` so an ordinary render reproduces the committed PDF). Projection checker: denied vault material (NOT-CLAIMABLE / PENDING-EVIDENCE / prose denials / Gaps & flags) fails closed for numbers, dates, URLs, skills, **and qualitative claims**; whole-token identity/URL/entry matching (no substring collisions); ownership-scoped, complete-value personal fields — email/phone/location **and now the candidate's own name and profile links** — checked against `## Basics`, not the whole vault; reversed-metric detection in either word order, plus dropped-negation ("no/zero/lack-of reduction") and worsened-outcome ("worsened"/"deteriorated"/"made X worse") tripwires. Evaluator: CRAFT gates run completion, with an explicit `## Run status` whose below-target-fit label is split by cause (a shippable coverage gap vs. a failed hard eligibility gate the candidate can't clear vs. an answerable unconfirmed gate); an evidence-based gate-status rule (persisted analysis → attached facts → user statement → unconfirmed, with a conflict/staleness rule); a `meta.target_level` so a no-JD run reads competitiveness at the intended seniority, not just field style; and a page-economy (dense-but-underfull) craft criterion. Reports — projection output, README tallies, and the eval-report's L0/L2 metrics — are freshness-tested against live script runs. **Known gaps, disclosed not hidden:** (1) truth-checking is lexical — a semantic synonym swap in a long line, or a paraphrase of a denied achievement, can still pass; the always-on claim→source pairing table plus mandatory human/evaluator attestation is the guarantee, the mechanical tripwires are defense-in-depth; (2) the suite exercises the deterministic tooling, not live agent-following-contract behavior end-to-end — that needs a model-in-the-loop eval harness, still unbuilt
