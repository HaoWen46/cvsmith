...
// right margin. A meta column that's *always* right-aligned to the same
// x, with nothing else ever occupying the gap between it and the body
// text, reads as an independent column to poppler's default (non
// -layout) text extraction once a section is sparse enough that no
// other line ever crosses that gap — the whole "column" then gets
// flushed to the end of the page, after later sections' headings, even
// though the visible PDF is correctly ordered. Tying the meta text's x
// position to the body text (small fixed gap, not a 1fr-vs-auto grid)
// keeps it from ever lining up into a repeatable column, at any
// content density. See evals/test_sparse_density.py.
//
// Round-6 (external review finding 7): a real right-aligned rail was
// re-tried two ways — a single-line `h(1fr)` fill and a grid cell with
// an inline `align(right)` sub-box — and both reproduce the exact
// sparse-density misordering this comment describes, on every
// template, because poppler's column heuristic keys on final glyph
// geometry (a consistent right edge with an uncrossed gap), not on
// which Typst construct produced it. Extraction order is the hard
// invariant, so the rail stays inline.
//
...
// fixture), the line rendered as a lone "\u{00b7} date", a punctuation
// mark with nothing attached to it, at the left margin. That reads as
// broken, not as metadata. The round-8 fix: drop the marker character
// entirely and keep only the small fixed gap. Extraction order is
// unaffected (the gap was always what kept this construct out of
// poppler's column heuristic, not the dot — confirmed in the scratch
// repro, cand_c_sparse: same fixed-gap construct minus the dot still
// extracts in-order). The gap alone, plus this text's distinct
// muted/italic styling, is enough to read as "this is the entry's
// meta," and reads as a stable, intentional position on every entry —
// never a detached, contextless marker.
//
// Round-9 (finding 9, reopened — user: "the date somehow isn't
// properly placed on the right hand side ... aesthetically bad"):
// round-8 only fixed the detached-marker half; dates still landed at a
// 40-500pt scatter with nothing near the right margin (measured fresh
// against this file and examples/ai-ml-intern/resume.pdf). The
...
// whole line width alone) DOES reproduce the reading-order regression
// — reconfirmed fresh this round (scratch: dates9/expt2) — so that
// case (and the rare atomic-wrap-alone-under-an-extreme-length-name
// case, e.g. long-meta's Education entry here) deliberately keeps the
// round-8 fixed-gap, left-flush fallback below instead of the leader.
// See evals/test_sparse_density.py's round-9 section for the
// right-alignment test.
// Round-9 follow-up (external review finding 9, still-open half): the
// doctrine above claimed the extreme-length-name case's "leader+date,
// boxed atomically... wraps whole to its own new line", but no code
// ever enforced that — `box(width: 1fr, ...)` is not nested inside the
// atomic date box, so Typst is free to lay the leader's repeated "."
// glyphs out on the CURRENT line (running to the right margin) and
// push only the date box down to a new line by itself: the exact
// "punctuation with nothing attached to it" defect finding 9
// originally named, reproduced with a leader instead of a "·".
// Reproduced against the committed long-meta fixture: this template's
// Education row rendered "...Engineering ......................." (x1
// 567.4, nothing after it) then "Sep 2022 – Jun 2026" alone at x0=41.1
// on the next line. `box(width: 1fr)` also can't simply nest one level
// deeper inside an outer atomic box — 1fr needs paragraph-level
// context to resolve, which an inner box doesn't provide.
//
...
// content's and the date box's natural (single-line) widths under the
// same ambient text styles. If left-side + a minimum viable leader +
// both small gaps + the date box doesn't fit — which also naturally
// covers left-side alone already needing multiple lines — row() falls
// back to round-8's plain fixed-gap construct (no leader) instead of
// emitting one that might dangle. The fallback's atomic date box still
// wraps to its own line when needed, but left-flush with no leader
// preceding it, so there is never a trailing run of periods with
// nothing after it. Verified against the long-meta fixture: the
// Education/Experience rows that used to split now render on one line
// via the fallback. See
...
// internally (finding 7, round-7). Only safe for bounded content like
// date ranges — an unbounded string (e.g. a long location) boxed the
// same way would run past the page edge instead of wrapping, so
// row() callers pass atomic: true only for dates.
#let row(left-side, right-side, atomic: false) = block(width: 100%)[
  #if left-side != none and right-side != none [
    #layout(size => {
      let gap = 0.35em
      let min-leader = 12pt
      let meta = text(size: 9.2pt, fill: muted, right-side)
    ...
    let meta = [#h(0.6em)#text(size: 9.2pt, fill: muted, right-side)]
