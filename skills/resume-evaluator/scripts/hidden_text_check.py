...
# ]
# ///
"""L2 — check whether visible page ink agrees with extracted text.
...
PASS. `raster_available` is stated either way so a caller never infers
that an omitted check passed.
...
def is_decorative(text: str) -> bool:
    """Return whether an extraction token is punctuation-only decoration."""
...
def main() -> int:
    ...
    if images is None:
        ...
        report.add("raster_available", PASS,
                   f"poppler rasterized {len(images)} page(s) at {args.dpi} "
                   "dpi — the cross-modal ink check ran")
